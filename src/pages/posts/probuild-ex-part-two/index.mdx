...
import _1_canon_diagram from './1-canon-diagram.png'
...
<SingleLightbox src={_1_canon_diagram} alt="Canons diagram mrdotb" />
...
_A test fixture is a fixed state of a set of objects used as a baseline for running tests. The purpose of a test fixture is to ensure that there is a well known and fixed environment in which tests are run so that results are repeatable._
...
_We use `:timer.sleep()` to simulate a HTTP request_
...
Refresh your riot token from their [dashboard](https://developer.riotgames.com/) and put it on `config/dev.local.exs`. _Dev token only last 24 hours_ like we did in part one.
...
Let's run our `Canon.Games` in `iex -S mix` and start collecting games. You can put one of the `platflorm_id` from list `["euw1", "jp1", "kr", "na1", "br1"]`. _You need to have some pro summoners in database by running the `Canon.Pros` like we did before_
...
_A GenServer is a process like any other Elixir process and it can be used to keep state, execute code asynchronously and so on._
